export function shapeSourceRead(inputs) {
  const source = record(inputs.crm_source);
  const fetched = record(inputs.fetched);
  const kind = stringValue(source.kind) ?? "web_fetch";
  if (kind !== "web_fetch") {
    throw new Error(`reconcile-live only supports crm_source.kind web_fetch, got ${kind}`);
  }
  const blockers = Array.isArray(fetched.blockers) ? fetched.blockers : [];
  if (blockers.length > 0) {
    throw new Error(`web.fetch blocked the connector export: ${blockers.join(", ")}`);
  }
  const status = typeof fetched.status === "number" ? fetched.status : 0;
  if (status < 200 || status >= 300) {
    throw new Error(`connector export fetch returned status ${status}`);
  }
  const body = typeof fetched.extracted === "string" ? fetched.extracted : "";
  if (!body) {
    throw new Error("connector export returned an empty body");
  }
  let parsed;
  try {
    parsed = JSON.parse(body);
  } catch {
    throw new Error("connector export is not valid JSON");
  }
  const records = Array.isArray(parsed)
    ? parsed
    : Array.isArray(parsed.crm_records)
      ? parsed.crm_records
      : Array.isArray(parsed.records)
        ? parsed.records
        : null;
  if (!records) {
    throw new Error("connector export carries no records array");
  }
  return {
    source_read: {
      read_kind: "web_fetch",
      fetched_ref: stringValue(fetched.final_url) ?? stringValue(source.url) ?? "unknown",
      records,
      fetch_status: status,
      content_digest: stringValue(fetched.content_digest) ?? null,
      truncated: record(fetched.provenance).truncated === true,
    },
  };
}
