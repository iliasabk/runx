...
### `reconcile-live` runner

A second runner, `reconcile-live`, runs the same pipeline with the source
read bound to governed tools directly instead of an agent task: a
`read-source-fetch` step executes `web.fetch` against
`crm_source.url` inside the run, and `read-source` shapes the fetched
connector export into the `source_read` contract. It supports
`{kind: web_fetch, url}` sources only; use it when the receipt must show the
source read executing inside the run without a managed agent.
